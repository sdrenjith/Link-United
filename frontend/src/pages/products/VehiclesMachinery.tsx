...
export default function VehiclesMachinery() {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const data = await productsService.list("vehicles-machinery");
        setProducts(data);
      } finally {
        setLoading(false);
      }
        ...
        <Container>
          <ScrollReveal>
            <p className="mx-auto max-w-3xl text-center text-base leading-relaxed text-[#888888]">
              We facilitate the international trade of heavy equipment, vehicles, and industrial
              machinery — connecting manufacturers with buyers across emerging and established
              markets worldwide.
            </p>
          </ScrollReveal>
